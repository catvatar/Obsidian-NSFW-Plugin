...
export default class ObsidianNSFW extends Plugin {
	...
	async toggleVisibility() {
		...
		if (new_visibility) {
			...
			this.settings.whereAreMyFiles.forEach((file) => {
				this.app.vault.adapter.rename(
					normalizePath(file.toPath),
					normalizePath(file.fromPath)
				);
			});
			...
			searchResult.forEach((_, key) => {
				...
				const newPath = normalizePath(
					this.settings.isolation + key.name
				);

				this.settings.whereAreMyFiles.push(
					new FileFromTo(oldPath, newPath)
				);
		...
		if (this.settings.toggleNotice) {
			new Notice(
				new_visibility ? "NSFW files visible" : "NSFW files NOT visible"
			);
		}
